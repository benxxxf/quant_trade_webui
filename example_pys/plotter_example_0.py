...
class DataframePlotterExample0(DataFramePlotter):
    ...
    def __init__(self):
        ...
        merged_df, df_list = local_parser.create_df_from_database(
            ...
            file_sql_appendcmd={
                f"{exe_dir_path}/example_database/国内指数日行情文件_19901219_20260729.parquet": "WHERE 交易所指数代码='1'",
                f"{exe_dir_path}/example_database/国内股票指数日市值文件_19910715_20260729.parquet": "WHERE 交易所指数代码='1'",
            },
        )
    ...
    def create_fig(self):
        ...
        volume_amount_color_list = [
            self.pos_color if val > 0 else self.neg_color for val in return_rate_series
        ]
        ...
        time_ind, price_centers, chip_matrix, chip_matrix_norm = get_chips(
            volume_amount_series=self.df["成份证券成交金额"],
            market_cap_series=self.df["样本股组合流通市值"],
            close_price_series=self.df["收盘指数"],
            low_price_series=self.df["最低指数"],
            high_price_series=self.df["最高指数"],
            price_bins=100,
            filter_date_start=self.time_start_str,
            filter_date_end=self.time_end_str,
        )
        close_thresholds = self.get_ranged_df()["收盘指数"].values[:, np.newaxis]
        # color_matrix = np.where(price_centers > close_thresholds, self.neg_color,self.pos_color)
        self.add_dynamic_bar_to_plot(
            name="筹码分布",
            time_index=time_ind,
            price_center_array=price_centers,
            chip_2d_array=chip_matrix_norm,
            threholds_value_array=self.get_ranged_df()["收盘指数"].to_numpy(),
            greater_pos_color_flag=False,
        )
        return self.fig
